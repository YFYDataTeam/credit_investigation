...
const PstAnalysis = ({ endPoint, companyId }) => {
  ...
  const label_agreement =
    pstAnalysis.annualAgreement.map((item) => item.agreement_end_year) || [];
  const annual_total_agreement_amount =
    pstAnalysis.annualAgreement.map((item) => item.total_agreement_amount) ||
    [];
  const annual_agreement_count =
    pstAnalysis.annualAgreement.map((item) => item.agreement_count) || [];
  return (
    <Container title="動產擔保分析">
      <AnnualAgreementPlot
